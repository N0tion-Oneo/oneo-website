...
function StatusBadge({ status }: { status: ProfileSuggestion['status'] }) {
  const config = {
    ...
    resolved: { icon: Check, color: 'bg-green-100 text-green-700', label: 'Resolved' },
    declined: { icon: XCircle, color: 'bg-red-100 text-red-700', label: 'Declined' },
    closed: { icon: AlertCircle, color: 'bg-gray-100 text-gray-600', label: 'Closed' },
  }
...
export default function SuggestionsPanel({
  ...
  return (
    <>
      ...
      <div className="fixed top-0 h-full w-80 bg-white shadow-xl z-[202] flex flex-col border-r border-gray-200"
        style={{ right: '50%' }}
      >
        {/* Header */}
        <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-[14px] font-semibold text-gray-900">Suggestions</h3>
              <p className="text-[12px] text-gray-500 mt-0.5">
                {fieldLabel}
                {relatedObjectLabel && (
                  <span className="text-gray-400"> - {relatedObjectLabel}</span>
                )}
              </p>
            </div>
            <button
        ...
        {/* Add Suggestion Form */}
        <form onSubmit={handleSubmit} className="px-4 py-3 border-b border-gray-200 bg-white">
          ...
          <div className="flex gap-2">
            <textarea
              ...
              className="flex-1 text-[13px] px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent resize-none"
              rows={2}
            />
            <button
              type="submit"
              disabled={!suggestionText.trim() || isCreating}
              className="self-end px-3 py-2 bg-gray-900 text-white rounded-md hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filteredSuggestions.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-[13px] text-gray-500">No suggestions yet</p>
              ...
              {filteredSuggestions.map((suggestion) => {
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[11px] font-medium text-gray-700">{suggestionFieldLabel}</span>
                  </div>

                  {/* Current value */}
                  {currentValue && (
                    <div className="mb-2 px-2 py-1.5 bg-white border border-gray-200 rounded text-[11px] text-gray-600 italic">
                  ...
                  {suggestion.status === 'declined' && suggestion.resolution_note && (
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <p className="text-[11px] text-gray-500 font-medium mb-1">Decline reason:</p>
