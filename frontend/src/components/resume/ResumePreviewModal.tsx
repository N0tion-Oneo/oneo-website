            ...
            {activeTab === 'profile' && (
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12px] font-medium text-gray-500 uppercase tracking-wide mb-1">
                      First Name
                    </label>
                ...
                {data.experiences.length === 0 ? (
                  ...
                  data.experiences.map((exp, index) => (
                    ...
                    >
                      ...
                      {exp.description && (
                        <p className="text-[13px] text-gray-600 mb-3 whitespace-pre-line">
                          {exp.description}
                        </p>
                      )}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5 mt-1">
                            {exp.skills.map((skill, i) => (
                              <span
                                key={i}
                                className="px-2 py-0.5 text-[11px] bg-purple-50 text-purple-700 rounded"
                              >
                                {skill}
                              </span>
                            ))}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  {data.all_technologies.length === 0 ? (
                    <p className="text-[13px] text-gray-500">No technologies extracted</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {data.all_technologies.map((tech, index) => (
                        <span
                          key={index}
                          className="px-3 py-1.5 text-[13px] bg-blue-50 text-blue-700 rounded-md border border-blue-100"
                        >
                          {tech}
                        </span>
                      ))}
                ...
                <div>
                  ...
                  {data.all_skills.length === 0 ? (
                    <p className="text-[13px] text-gray-500">No skills extracted</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {data.all_skills.map((skill, index) => (
                        <span
                          key={index}
                          className="px-3 py-1.5 text-[13px] bg-purple-50 text-purple-700 rounded-md border border-purple-100"
                        >
                          {skill}
                        </span>
                      ))}
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2 text-[13px] font-medium text-gray-700 bg-white border border-gray-200 rounded-md hover:bg-gray-50 disabled:opacity-50 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isImporting}
                className="inline-flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-white bg-gray-900 rounded-md hover:bg-gray-800 disabled:opacity-50 transition-colors"
              >
                {isImporting ? (
                  <>
                    <svg
