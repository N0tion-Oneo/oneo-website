    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <main className="max-w-5xl mx-auto px-6 py-10">
          {/* Page Header */}
          <div className="mb-8">
            <h1 className="text-[26px] font-semibold text-gray-900 dark:text-gray-100">Talent Directory</h1>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
          <div className="grid md:grid-cols-4 gap-4">
            <div>
              <label className="block text-[12px] font-medium text-gray-500 dark:text-gray-400 mb-1.5">Search</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[12px] font-medium text-gray-500 dark:text-gray-400 mb-1.5">Seniority</label>
              <select
            ...
            <div>
              <label className="block text-[12px] font-medium text-gray-500 dark:text-gray-400 mb-1.5">Work Preference</label>
              <select
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full px-4 py-2 text-[13px] font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                Clear Filters
              </button>
            </div>
            ...
            {candidates.map((candidate: CandidateProfileSanitized) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-gray-900 dark:bg-gray-700 rounded-full flex items-center justify-center text-white dark:text-gray-100 text-[14px] font-medium flex-shrink-0">
                    {candidate.initials}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[15px] font-medium text-gray-900 dark:text-gray-100 truncate">
                    ...
                    {candidate.location && (
                      <p className="text-[12px] text-gray-400 dark:text-gray-500 mt-1 flex items-center gap-1">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {candidate.seniority && (
                    <span className="px-2 py-0.5 text-[11px] font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded">
                      {getSeniorityLabel(candidate.seniority)}
                    </span>
                  )}
                  {candidate.work_preference && (
                    <span className="px-2 py-0.5 text-[11px] font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded">
                      {getWorkPreferenceLabel(candidate.work_preference)}
                    </span>
                  )}
                  {candidate.years_of_experience && (
                    <span className="px-2 py-0.5 text-[11px] font-medium bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded">
                      {candidate.years_of_experience}
                    </span>
                  )}
                </div>
        ...
        {!isLoading && !error && (hasPrevious || hasNext) && (
          <div className="mt-8 flex items-center justify-center gap-4">
            <button
              ...
              className="px-4 py-2 text-[13px] font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Previous
            </button>
            <span className="text-[13px] text-gray-500 dark:text-gray-400">Page {filters.page}</span>
            <button
              onClick={() => setFilters((prev) => ({ ...prev, page: prev.page + 1 }))}
              disabled={!hasNext}
              className="px-4 py-2 text-[13px] font-medium text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
