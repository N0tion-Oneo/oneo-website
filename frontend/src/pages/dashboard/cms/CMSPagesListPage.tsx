...
export default function CMSPagesListPage() {
  ...
  const getTypeBadge = (type: PageType) => {
    const styles: Record<PageType, string> = {
      [PageType.LEGAL]: 'bg-purple-100 text-purple-700',
      [PageType.ABOUT]: 'bg-blue-100 text-blue-700',
      [PageType.SERVICE]: 'bg-cyan-100 text-cyan-700',
      [PageType.GENERIC]: 'bg-gray-100 text-gray-600',
    }
    return (
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[20px] font-semibold text-gray-900">Pages</h1>
              ...
              {filteredPages.map((page) => (
                <tr key={page.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <div>
                      <p className="text-[13px] font-medium text-gray-900">{page.title}</p>
                      {page.excerpt && (
                        <p className="text-[12px] text-gray-500 truncate max-w-[300px]">
                          {page.excerpt}
                        </p>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3">
                    <code className="text-[12px] text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded">
                      /{page.slug}
                    </code>
                  </td>
